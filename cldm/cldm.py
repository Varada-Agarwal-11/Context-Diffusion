...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.input_cond_block = TimestepEmbedSequential(
    ...
    def forward(self, x, timesteps, example_pair, query, context, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        # "****************what is going on here???! context = txt prompt, example pair: to be shifted images, query: example pair shouldnt be added to this"
        example_pair_hint = self.input_hint_block(example_pair, emb, context)
        query_hint = self.input_cond_block(query, emb, context)
        ...
        for module, zero_conv in zip(self.input_blocks, self.zero_convs):
            if query_hint is not None:
                h = module(h, emb, context) #h is passed through a module of input block, the guided hint is added and then set to none so same hint isnt added again 
                h += query_hint
                query_hint = None
            else:
                h = module(h, emb, context) #rest of the modules the h added to guided hint is passed through 
...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        control = self.control_model(x=x_noisy, timesteps=t,
                                     ...
                                     query=cond['query'][0], context=cond_txt)
        # "*******************************************************************************************************************************************"
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        if sample:
            # get denoise row
            samples, z_denoise_row = self.sample_log(cond={"query": [c_cat], "example_pair": [c_example_pair], "c_crossattn": [c]},
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_full = {"query": [c_cat], "example_pair": [c_example_pair], "c_crossattn": [uc_cross]}
            samples_cfg, _ = self.sample_log(cond={"query": [c_cat], "example_pair": [c_example_pair], "c_crossattn": [c]},
                                             batch_size=N, ddim=use_ddim,
                                             ddim_steps=ddim_steps, eta=ddim_eta,
