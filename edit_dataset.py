...
class EditDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> dict[str, Any]:
        ...
        example_seed = torch.randint(0, 1000, ()).item()
        ...
        image_0 = ds['train']['input'][example_seed]  # Assuming it's already a PIL image object

        image_1 = ds['train']['output'][example_seed]
        ...
        prompt = ds['train']['text'][example_seed]
        txt_log = prompt
input_prompt
        # Load Controls; shape -> h w c
        task = np.random.choice(['inv_seg'])
        # task = np.random.choice(['inv_seg', 'inv_depth', 'inv_hed', 'seg', 'depth', 'hed'])
        txt_log = task
        if task == 'inv_seg':
            image_seg = ds['train']['input'][example_seed]
            ...
            image_query = ds['train']['output'][example_seed]
